...
use WooCommerce_Postal_codes_validation\Engine;

/**
 * WooCommerce_Postal_codes_validation Initializer
 */
class Initialize {
	...
	private function load_classes() {
		$this->classes = \apply_filters( 'woocommerce_postal_codes_validation_classes_to_execute', $this->classes );

		foreach ( $this->classes as $class ) {
			try {
				$this->initialize_plugin_class( $class );
			} catch ( \Throwable $err ) {
				\do_action( 'woocommerce_postal_codes_validation_initialize_failed', $err );
	...
	private function get_classes( string $namespacetofind ) {
		...
		$namespacetofind = 'WooCommerce_Postal_codes_validation\\' . $namespacetofind;

		// In case composer has autoload optimized
		if ( isset( $classmap[ 'WooCommerce_Postal_codes_validation\\Engine\\Initialize' ] ) ) {
		...
		if ( isset( $prefix[ $namespacetofind ] ) ) {
			...
			if ( !WP_DEBUG ) {
				\wp_die( \esc_html__( 'WooCommerce Postal codes validation is on production environment with missing `composer dumpautoload -o` that will improve the performance on autoloading itself.', WPCV_TEXTDOMAIN ) );
			}
