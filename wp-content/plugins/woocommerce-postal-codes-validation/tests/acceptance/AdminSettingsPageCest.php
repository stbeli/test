...
class AdminSettingsPageCest {
	...
	function add_plugin_admin_menu(AcceptanceTester $I) {
		...
		$I->see('WooCommerce Postal codes validation Settings', 'h2');
	}

	function add_action_link(AcceptanceTester $I) {
		$I->wantTo('check plugin list page if include mine');
		$I->amOnPluginsPage();
		$I->see('WooCommerce Postal codes validation', 'tr.active[data-plugin="woocommerce-postal-codes-validation/woocommerce-postal-codes-validation.php"] td strong');
	}
